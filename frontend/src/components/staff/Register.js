...
function Register() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!viewOtp) {
      if (email && password) {
        try {
          const res = await fetch("http://127.0.0.1:5000/staff/auth/register", {
            method: "POST",
            headers: {
      ...
      if (otp) {
        try {
          const res = await fetch("http://127.0.0.1:5000/staff/auth/verify", {
            method: "POST",
  ...
  return (
    <div className="Register h-full flex flex-col justify-center items-center self-center hover:cursor-default px-3">
      ...
      <form
        className="bg-white p-3 m-2 max-w-full min-w-80 shadow-md rounded"
      ...
      <span className="flex w-full sm:max-w-[40%] justify-around text-md">
        ...
        <Link to="/staff/login" className="text-amber-500 font-semibold">
          Login
        </Link>
      </span>
      {error && (
